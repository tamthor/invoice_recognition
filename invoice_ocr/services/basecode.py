...
img_path = "doc/after_alignimage.png"
# img_path = "doc/TEST.png"
...
def process_text_data(text_elements, ma_hang_pos, so_luong_pos):
    ...
    ma_hang_data = []
    so_luong_data = []
    stt_width = 50  # Độ rộng cột STT (có thể điều chỉnh)
...
def save_cropped_columns(cropped_table, filtered_positions, output_dir='doc/croptable'):
...
def detect_text():
    ...
    result = extract_table_from_image(image, table_lines)
    if result[0] is None:
    ...
    # Lưu ảnh bảng đã cắt
    cv2.imwrite('doc/croptable/table.png', cropped_table)
    print(f"Đã lưu bảng đã cắt tại: doc/croptable/table.png")
    ...
    # Thêm biến để lưu thông tin nhà cung cấp và đơn hàng
    ma_ncc = None
    so_don_hang = None
    all_text_elements = []  # Lưu tất cả text elements trước khi lọc bảng

    # Xử lý và lưu tất cả text elements trước
    for page in response.full_text_annotation.pages:
        for block in page.blocks:
            for paragraph in block.paragraphs:
                for word in paragraph.words:
                    ...
                    # Chỉ thêm vào text_elements nếu nằm trong bảng
                    if (table_x <= center_x <= table_x + table_w and 
                        table_y <= center_y <= table_y + table_h):
                        text_elements.append(elem)

    # Tìm mã nhà cung cấp và số đơn hàng
    ma_ncc = None
    so_don_hang = None
