...
def detect_and_filter_columns(cropped_table, min_line_length_ratio=0.2, max_line_gap=10):
    '''Phát hiện và lọc vị trí các cột trong bảng'''

    # Chuyển sang ảnh xám và làm rõ đường kẻ
    gray = cv2.cvtColor(cropped_table, cv2.COLOR_BGR2GRAY)
    gray = cv2.GaussianBlur(gray, (3, 3), 0)
    _, thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)

    # Dùng Morphology để làm rõ đường kẻ dọc
    vertical_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, cropped_table.shape[0] // 25))
    vertical_lines = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, vertical_kernel, iterations=1)

    # Tìm đường thẳng bằng Hough Line
    edges = cv2.Canny(vertical_lines, 50, 150, apertureSize=3)
    lines = cv2.HoughLinesP(edges, 1, np.pi / 180, threshold=100,
                            minLineLength=int(cropped_table.shape[0] * min_line_length_ratio),
                            maxLineGap=max_line_gap)

    # Lọc các đường thẳng gần dọc
    vertical_positions = []
    ...
    if vertical_positions:
        ...
        for x in vertical_positions[1:]:
            if x - current_group[-1] < 10:  # khoảng cách gom nhóm
...
img_path = os.path.join(DOC_DIR, 'captured_image.jpg')
